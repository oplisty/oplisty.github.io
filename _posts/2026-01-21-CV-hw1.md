...
e can also use other distance metrics such as L1 distance.  
...
Which of the following preprocessing steps will **not** change the performance of a Nearest Neighbor classifier that uses L1 distance?  
Select all that apply. (Assume both training and test examples are preprocessed in the same way.)

1. Subtracting the mean \(\mu\) \(\big(\tilde{p}_{ij}^{(k)}=p_{ij}^{(k)}-\mu\big)\).
2. Subtracting the per-pixel mean \(\mu_{ij}\) \(\big(\tilde{p}_{ij}^{(k)}=p_{ij}^{(k)}-\mu_{ij}\big)\).
3. Subtracting the mean \(\mu\) and dividing by the standard deviation \(\sigma\).
4. Subtracting the pixel-wise mean \(\mu_{ij}\) and dividing by the pixel-wise standard deviation \(\sigma_{ij}\).
5. Rotating all images by the same angle (empty regions are padded with the same pixel value and no interpolation is performed).
