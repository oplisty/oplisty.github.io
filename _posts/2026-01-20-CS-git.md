...
- Git 会把你**工作区的改动**（默认也包括已暂存的改动）打包成一份“stash”，存到仓库里一个专门的栈中。
- 然后你的工作区会回到一个**干净状态**（就像你没改过一样），`git status` 往往会显示 `working tree clean`。
- 这些改动**没有丢**，只是被临时“收起来”了。
...
3. `git stash pop` 撤销 stash：什么时候会用到这个技巧？
